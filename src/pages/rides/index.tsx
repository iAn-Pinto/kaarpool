...
type ColumnType = ColumnDef<RideInterface, unknown>;

interface RideListPageProps {
...
export function RideListPage(props: RideListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getRides({
        relations: ['car', 'booking.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['start_location.contains', 'end_location.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RideInterface>>(
    () => `/rides?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: RideInterface) => {
    if (hasAccess('ride', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/rides/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'start_location', header: 'Start Location', accessorKey: 'start_location' },
    { id: 'end_location', header: 'End Location', accessorKey: 'end_location' },
    {
      id: 'start_time',
      header: 'Start Time',
    ...
    { id: 'seats_available', header: 'seats_available', accessorKey: 'seats_available' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/rides/view/${record.id}`} passHref legacyBehavior>
                <Button
