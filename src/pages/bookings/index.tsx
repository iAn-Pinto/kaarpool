...
export function BookingListPage(props: BookingListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getBookings({
        relations: ['ride', 'user', 'feedback.count'],
  ...
  const columns: ColumnType[] = [
    { id: 'seat_count', header: 'seat_count', accessorKey: 'seat_count' },
    ...
    hasAccess('ride', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/rides/view/${record.ride?.id}`}>
              {record.ride?.start_location}
            </Link>
          ),
        }
      : null,
