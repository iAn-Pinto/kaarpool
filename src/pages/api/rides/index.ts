...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getRidesPublic() {
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'ride');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.ride.count(countOptions as unknown),
  ...
  async function getRides() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.ride
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'ride'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
